...
TEST_F(File_test, CreatingFileWorks)
{
    ...
    auto sut = FileBuilder().open_mode(OpenMode::PurgeAndCreate).create(m_sut_file_path);
...
TEST_F(File_test, CreatingWithPermissionsWorks)
{
    ...
    ASSERT_FALSE(
        FileBuilder().open_mode(OpenMode::PurgeAndCreate).permissions(perms).create(m_sut_file_path).has_error());

    auto sut = FileBuilder().open_mode(OpenMode::OpenExisting).create(m_sut_file_path);
...
TEST_F(File_test, PurgeAndCreateRemovesExistingFile)
{
    ...
    auto sut2 = FileBuilder().open_mode(OpenMode::PurgeAndCreate).create(m_sut_file_path);
...
TEST_F(File_test, CreatingExclusivelyTwiceFails)
{
    ...
    auto sut = FileBuilder().open_mode(OpenMode::ExclusiveCreate).create(m_sut_file_path);
    ASSERT_FALSE(sut.has_error());

    auto sut2 = FileBuilder().open_mode(OpenMode::ExclusiveCreate).create(m_sut_file_path);
...
TEST_F(File_test, OpeningExistingFileWorks)
{
    ...
    auto sut = FileBuilder().open_mode(OpenMode::PurgeAndCreate).create(m_sut_file_path);
    auto sut2 = FileBuilder().open_mode(OpenMode::OpenExisting).create(m_sut_file_path);
...
TEST_F(File_test, OpenOrCreateCreatesNonExistingFile)
{
    ...
    auto sut = FileBuilder().open_mode(OpenMode::OpenOrCreate).create(m_sut_file_path);
...
TEST_F(File_test, OpenOrCreateOpensExistingFile)
{
    ...
    auto sut2 = FileBuilder().open_mode(OpenMode::OpenOrCreate).create(m_sut_file_path);
...
TEST_F(File_test, OpenFileForReadingWithInsufficientPermissionFails)
{
    ...
    auto sut2 =
        FileBuilder().open_mode(OpenMode::OpenExisting).access_mode(AccessMode::ReadOnly).create(m_sut_file_path);
    ASSERT_TRUE(sut2.has_error());
...
TEST_F(File_test, OpenFileForReadWriteWithInsufficientPermissionFails)
{
    ...
    auto sut = FileBuilder().open_mode(OpenMode::PurgeAndCreate).permissions(perms::owner_read).create(m_sut_file_path);
    ASSERT_FALSE(sut.has_error());

    auto sut2 =
        FileBuilder().open_mode(OpenMode::OpenExisting).access_mode(AccessMode::ReadWrite).create(m_sut_file_path);
    ASSERT_TRUE(sut2.has_error());
    EXPECT_THAT(sut2.error(), Eq(FileCreationError::PermissionDenied));
}
#endif
...
TEST_F(File_test, RemoveReturnsTrueWhenFileExist)
{
    ...
    {
        EXPECT_FALSE(FileBuilder().open_mode(OpenMode::PurgeAndCreate).create(m_sut_file_path).has_error());
    }
