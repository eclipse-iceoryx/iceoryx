...
template <uint64_t Capacity>
template <typename T>
inline typename std::enable_if<(is_cxx_string<T>::value || std::is_same<T, std::string>::value), bool>::type
string<Capacity>::operator==(const T& rhs) const noexcept
{
    return (compare<T>(rhs) == 0);
}
