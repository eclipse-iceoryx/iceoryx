...
/// @param error error object (or code)
    ...
    #define IOX_DEBUG_ASSERT(expr) IOX_REPORT_IF(!(expr), \
      Violation(eh3::DEBUG_ASSERT_VIOLATION_CODE), DEBUG_ASSERT_VIOLATION)
#else
    // the proxy is not actually constructed (false branch)
    #define IOX_DEBUG_ASSERT(expr) if(false) eh3::ErrorProxy<eh3::DebugAssertViolation>()
#endif
...
#define IOX_PANIC(...) do { eh3::panic(__VA_ARGS__); } while(false)
...
#ifdef DEBUG
    #define IOX_EXPECTS(expr) IOX_REPORT_IF(!(expr), Violation(eh3::PRECONDITION_VIOLATION_CODE), PRECONDITION_VIOLATION)
#else
