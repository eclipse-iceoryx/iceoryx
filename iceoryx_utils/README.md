...
| [design_pattern](#design\ pattern) | Certain code patterns which are repeating themself all over the code are abstracted and stored in here. At the moment we only have the creation pattern but maybe there will be more. |
...
|```poor_mans_heap```     |   |   | Acquires memory on the stack for placement new instatiations. All classes must inherit from a base class which has to be known at compile time but the class itself does not have to be known - only the size. |
...
|```LockedLoFFLi```       | i | X | LIFO based index manager. One building block of our memory manager. After construction it contains the indices {0 ... n} which you can acquire and release. |
...
|```SoFi```               | i |   | Single Producer, Single Consumer Lock Free Safely Overflowing FiFo |
|```TACO```               | i |   | Solution if you would like to use ```std::atomic``` with data types larger than 64 bit. Wait free data synchronization mechanism between threads.|
...
|```SharedMemoryObject``` | i | | Creates and mappes existing shared memory into the application. |
