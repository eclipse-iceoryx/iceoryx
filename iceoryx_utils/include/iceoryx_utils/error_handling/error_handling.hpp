...
#define ICEORYX_ERRORS(error) \
    ...
    error(POSH__SENDERPORT_NOT_UNIQUE) \
    error(POSH__SENDERPORT_ROUDI_MIDDLEWARESENDERLIST_FULL) \
    error(POSH__SENDERPORT_CREATION_UNDEFINED_BEHAVIOR) \
    ...
    error(POPO__CHUNK_DISTRIBUTOR_LOCKING_ERROR) \
    ...
    error(POPO__CONDITION_VARIABLE_WAITER_FAILED_TO_CREATE_SEMAPHORE) \
    ...
    error(PORT_MANAGER__INTERFACE_FIFO_OVERFLOW) \
