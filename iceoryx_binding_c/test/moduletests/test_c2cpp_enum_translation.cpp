...
TEST(c2cpp_enum_translation_test, ServerRequestResult)
{
    ::testing::Test::RecordProperty("TEST_ID", "47c0a378-5bad-4161-b5b7-8ee0e2fae9f9");
    constexpr EnumMapping<iox::popo::ServerRequestResult, iox_ServerRequestResult> EVENTS[]{
        {iox::popo::ServerRequestResult::TOO_MANY_REQUESTS_HELD_IN_PARALLEL,
         ServerRequestResult_TOO_MANY_REQUESTS_HELD_IN_PARALLEL},
        {iox::popo::ServerRequestResult::NO_PENDING_REQUESTS, ServerRequestResult_NO_PENDING_REQUESTS},
        {iox::popo::ServerRequestResult::UNDEFINED_CHUNK_RECEIVE_ERROR,
         ServerRequestResult_UNDEFINED_CHUNK_RECEIVE_ERROR},
        {iox::popo::ServerRequestResult::NO_PENDING_REQUESTS_AND_SERVER_DOES_NOT_OFFER,
    ...
    for (const auto event : EVENTS)
    {
        ...
        case iox::popo::ServerRequestResult::TOO_MANY_REQUESTS_HELD_IN_PARALLEL:
            EXPECT_EQ(c2cpp::serverRequestResult(event.c), event.cpp);
            break;
...
// which is tested and handled here
// explicitly commented out since we are testing undefined behavior here and that we
// return the default value NO_PENDING_REQUESTS always in the undefined behavior case
// the clang sanitizer detects this successfully and this leads to termination, and with this the test fails
#if !defined(__clang__)
    iox::Error errorValue = iox::Error::kNO_ERROR;
    auto errorHandlerGuard = iox::ErrorHandler::setTemporaryErrorHandler(
        [&](const iox::Error e, const std::function<void()>, const iox::ErrorLevel) { errorValue = e; });
    EXPECT_EQ(c2cpp::serverRequestResult(static_cast<iox_ServerRequestResult>(-1)),
              iox::popo::ServerRequestResult::NO_PENDING_REQUESTS);
    EXPECT_THAT(errorValue, Eq(iox::Error::kBINDING_C__C2CPP_ENUM_TRANSLATION_INVALID_SERVER_REQUEST_RESULT_VALUE));
#endif
#pragma GCC diagnostic pop
}
} // namespace
