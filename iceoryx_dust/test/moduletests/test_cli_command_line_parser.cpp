                 ...
                 const std::vector<std::string>& requiredValuesToRegister = {}) noexcept
{
    ...
    bool wasErrorHandlerCalled = false;
    {
        ...
        {
            optionSet.addOptional(o[0], iox::cxx::convert<std::string, OptionName_t>(o), "", "int", "0");
        }
        for (const auto& s : switchesToRegister)
        {
            optionSet.addSwitch(s[0], iox::cxx::convert<std::string, OptionName_t>(s), "");
        }
        for (const auto& r : requiredValuesToRegister)
        {
            optionSet.addRequired(r[0], iox::cxx::convert<std::string, OptionName_t>(r), "", "int");
        }
    ...
    bool wasErrorHandlerCalled = false;
    {
        ...
        for (const auto& o : optionsToRegister)
        {
            optionSet.addOptional(
                o[0], iox::cxx::convert<std::string, OptionName_t>(o), "", "int", CommandLineParser_test::defaultValue);
        }
        for (const auto& s : switchesToRegister)
        {
            optionSet.addSwitch(s[0], iox::cxx::convert<std::string, OptionName_t>(s), "");
