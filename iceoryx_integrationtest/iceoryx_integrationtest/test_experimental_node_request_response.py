...
def generate_test_description():
    ...
    executable_list = ['iox-cpp-node-server', 'iox-cpp-node-client']
    process_list = []
    ...
    ]), {'iox-cpp-node-server': process_list[0],
         'iox-cpp-node-client': process_list[1]}
...
class TestWaitSetExample(unittest.TestCase):
    ...
    def test_server(self, proc_output):
        proc_output.assertWaitFor(
            'Got Request: 21 + 34', timeout=45, stream='stdout')

    def test_client(self, proc_output):
        proc_output.assertWaitFor(
            'Got Response : 55', timeout=45, stream='stdout')
