...
# @brief Test goal: "Integrationtest for the ara_com example of iceoryx"
...
def generate_test_description():
    ...
    executable_list = ['iox-cpp-ara-publisher', 'iox-cpp-ara-subscriber']
    process_list = []
    ...
    ]), {'iox-cpp-ara-publisher': process_list[0], 'iox-cpp-ara-subscriber': process_list[1],
...
class TestAraComExample(unittest.TestCase):
    ...
    def test_publisher_subscriber_data_exchange(self, proc_output):
        proc_output.assertWaitFor(
            'iox-cpp-ara-publisher sent values: 104, 159, 248, 15, 392', timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-cpp-ara-subscriber got value: 104', timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-cpp-ara-subscriber got value: 159', timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-cpp-ara-subscriber got value: 248', timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-cpp-ara-subscriber got value: 392', timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-cpp-ara-asubscriber got value: 15', timeout=45, stream='stdout')
