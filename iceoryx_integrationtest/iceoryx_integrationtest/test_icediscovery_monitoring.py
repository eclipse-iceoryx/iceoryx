...
# @brief Test goal: "Integrationtest for the monitoring features of the icediscovery example"
...
class TestIcediscoveryMonitoringExample(unittest.TestCase):
    def test_roudi_ready(self, proc_output):
        proc_output.assertWaitFor(
            'RouDi is ready for clients', timeout=45, stream='stdout')

    def test_wait_for_service(self, proc_output):
        proc_output.assertWaitFor(
            'iox-wait-for-service waiting for service <Camera, FrontLeft, Image>',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-wait-for-service <Camera, FrontLeft, Image> was available',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-wait-for-service waiting for any discovery change',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-wait-for-service discovery changed. Searching <Camera, FrontLeft, Image>',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-wait-for-service <Camera, FrontLeft, Image> was unavailable',
            timeout=45, stream='stdout')

    def test_discovery_monitoring(self, proc_output):
        proc_output.assertWaitFor(
            'iox-monitor-discovery <Camera, FrontLeft, Image> available',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-monitor-discovery search result:empty',
            timeout=45, stream='stdout')
        proc_output.assertWaitFor(
            'iox-monitor-discovery Camera, FrontLeft, Image',
            timeout=45, stream='stdout')
