                                ...
                                const version::VersionInfo& versionInfo) noexcept
{
    ...
    m_processList.emplace_back(name, pid, *dataSegmentMemoryManager, isMonitored, dataSegmentId, sessionId);
    ...
    searchForProcessAndThen(
        ...
        [&](Process& process) { // create a PublisherPort
            auto maybePublisher = m_portManager.acquirePublisherPortData(
                service, publisherOptions, name, &process.getDataSegmentMemoryManager(), portConfigInfo);
