...
TEST_F(PortManager_test, doDiscovery_singleShotPublisherFirst)
{
    PublisherPortUser publisher(
        m_portManager
            ->acquirePublisherPortData({1, 1, 1}, 1, "guiseppe", m_payloadMemoryManager, "node", PortConfigInfo())
            .value());
    ...
    SubscriberPortUser subscriber(
        m_portManager->acquireSubscriberPortData({1, 1, 1}, 1, "schlomo", "node", PortConfigInfo()).value());
...
TEST_F(PortManager_test, doDiscovery_rightOrdering)
{
    ...
    SubscriberPortUser subscriber2(
        m_portManager->acquireSubscriberPortData({1, 1, 1}, 1, "ingnatz", "node", PortConfigInfo()).value());
...
TEST_F(PortManager_test, SubscriberOverflow)
{
    ...
    for (unsigned int i = 0; i < subForP1; i++)
    {
        auto subscriberPortDataResult =
            m_portManager->acquireSubscriberPortData(getUniqueSD(), 1, p1, r1, PortConfigInfo());
...
TEST_F(PortManager_test, PortDestroy)
{
    ...
    auto publisherData1 =
        m_portManager->acquirePublisherPortData(cap1, 1, p1, m_payloadMemoryManager, "node", PortConfigInfo()).value();
    auto subscriberData1 = m_portManager->acquireSubscriberPortData(cap2, 1, p1, "node", PortConfigInfo()).value();

    auto publisherData2 =
        m_portManager->acquirePublisherPortData(cap2, 1, p2, m_payloadMemoryManager, "node", PortConfigInfo()).value();
    auto subscriberData2 = m_portManager->acquireSubscriberPortData(cap1, 1, p2, "node", PortConfigInfo()).value();
    ...
    publisherData2 =
        m_portManager->acquirePublisherPortData(cap2, 1, p2, m_payloadMemoryManager, "node", PortConfigInfo()).value();
    subscriberData2 = m_portManager->acquireSubscriberPortData(cap1, 1, p2, "node", PortConfigInfo()).value();
