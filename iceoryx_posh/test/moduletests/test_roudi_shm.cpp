...
using ReceiverPort = iox::popo::SubscriberPortUser;
using SenderPort = iox::popo::PublisherPortUser;
...
TEST_F(PortManager_test, doDiscovery_singleShotSenderFirst)
{
    ...
    ReceiverPort receiver1(
        m_shmManager->acquireSubscriberPortData({1, 1, 1}, 1, "/schlomo", "runnable", PortConfigInfo()).get_value());
...
TEST_F(PortManager_test, doDiscovery_singleShotReceiverFirst)
{
    ...
    /// @todo #252 re-add receiver handler for v1.0?
    // ASSERT_THAT(sender.getMembers()->m_receiverHandler.m_receiverVector.size(), Eq(1u));
    // auto it = sender.getMembers()->m_receiverHandler.m_receiverVector.begin();
...
TEST_F(PortManager_test, doDiscovery_rightOrdering)
{
    ...
    ReceiverPort receiver2(
        m_shmManager->acquireSubscriberPortData({1, 1, 1}, 1, "/ingnatz", "runnable", PortConfigInfo()).get_value());
...
TEST_F(PortManager_test, SenderReceiverOverflow)
{
    ...
    std::vector<iox::popo::PublisherPortData*> avaSender1(pubForP1);
    std::vector<iox::popo::SubscriberPortData*> avaReceiver1(subForP1);


    for (unsigned int i = 0; i < pubForP1; i++)
    {
        auto sen =
            m_shmManager->acquirePublisherPortData(getUniqueSD(), 1, p1, m_payloadMemoryManager, r1, PortConfigInfo());
    ...
    {
        auto rec = m_shmManager->acquireSubscriberPortData(getUniqueSD(), 1, p1, r1, PortConfigInfo());
        ASSERT_THAT(rec.has_error(), Eq(false));
        avaReceiver1[i] = rec.get_value();
    }
...
TEST_F(PortManager_test, PortDestroy)
{
    ...
    auto senderData1 =
        m_shmManager->acquirePublisherPortData(cap1, 1, p1, m_payloadMemoryManager, "runnable", PortConfigInfo())
            .get_value();
    auto receiverData1 = m_shmManager->acquireSubscriberPortData(cap2, 1, p1, "runnable", PortConfigInfo()).get_value();

    auto senderData2 =
        m_shmManager->acquirePublisherPortData(cap2, 1, p2, m_payloadMemoryManager, "runnable", PortConfigInfo())
            .get_value();
