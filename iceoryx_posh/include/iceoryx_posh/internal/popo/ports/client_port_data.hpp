...
struct ClientPortData : public BasePortData
{
    ...
    ClientServerPortTypes::ClientChunkSenderData_t m_chunkSenderData;
    ClientServerPortTypes::ClientChunkReceiverData_t m_chunkReceiverData;
    std::atomic_bool m_offeringRequested{false};
    std::atomic_bool m_offered{false};

    std::atomic_bool m_connectRequested{false};
    std::atomic<SubscribeState> m_connectionState{ConnectionState::NOT_SUBSCRIBED};
};
