...
class MemoryManager {
  using MaxSize_t =
      cxx::range<uint32_t, 1,
                 std::numeric_limits<uint32_t>::max() - sizeof(ChunkHeader)>;
...
public:
  ...
  static uint64_t
  requiredManagementMemorySize(const MePooConfig &f_mePooConfig);
  static uint64_t requiredFullMemorySize(const MePooConfig &f_mePooConfig);

private:
  void printMemPoolVector() const;
  void
  addMemPool(posix::Allocator *f_managementAllocator,
             posix::Allocator *f_payloadAllocator,
             const cxx::greater_or_equal<uint32_t, MemPool::MEMORY_ALIGNMENT>
                 f_payloadSize,
