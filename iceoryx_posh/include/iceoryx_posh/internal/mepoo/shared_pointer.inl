    ...
    : m_chunk(chunk)
{
    ...
    {
        new (chunk.m_chunkManagement->m_chunkInfo->m_payload) T(std::forward<Targs>(args)...);
...
inline void SharedPointer<T>::deleteManagedObjectIfNecessary()
{
    ...
    {
        static_cast<T*>(m_chunk.m_chunkManagement->m_chunkInfo->m_payload)->~T();
    }
}
...
{
    return static_cast<T*>(m_chunk.m_chunkManagement->m_chunkInfo->m_payload);
}
